...
export default function CardItem({ card, isDragging = false, onUpdate, onDelete }: Props) {
    ...
    return (
      <div className="bg-void border border-accent/40 rounded-xl p-3"
        ...
        <textarea
          ...
          className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-xs text-textDim focus:outline-none focus:border-accent transition-colors resize-none mb-3"
          rows={3}
        />
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className="text-xs text-textDim px-3 py-1.5 rounded-lg hover:bg-surface transition-colors">
            İptal
          </button>
          <button
            onClick={() => { if (confirm('Kartı sil?')) onDelete?.() }}
            className="ml-auto text-xs text-red-500/50 hover:text-red-400 px-2 py-1.5 rounded-lg hover:bg-red-950/20 transition-colors">
            Sil
          </button>
        </div>
      </div>
  ...
  return (
    <div
      ref={setNodeRef}
      style={{ ...style, ...(isDragging ? { boxShadow: '0 10px 40px rgba(192,132,252,0.3)' } : {}) }}
      {...attributes}
      {...listeners}
      onDoubleClick={() => setIsEditing(true)}
      className={`group bg-surface border border-border rounded-xl p-3 cursor-grab active:cursor-grabbing transition-all hover:border-accent/40 ${isDragging ? 'rotate-2 scale-105 border-accent/60' : ''}`}
    >
      <p className="text-sm font-medium text-textMain group-hover:text-accent transition-colors leading-snug">
        {card.title}
      </p>
      {card.description && (
        <p className="text-xs text-textDim mt-1.5 line-clamp-2 leading-relaxed">
          {card.description}
        </p>
      )}
